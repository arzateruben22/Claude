// YourBusiness — Supabase Edge Function: send-confirmation
//
...
async function sendEmail(to: string, subject: string, html: string) {
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: Deno.env.get("FROM_EMAIL") ?? "YourBusiness <hello@lumevina.com>",
      to, subject, html,
    }),
  });
}
...
async function messageFor(sb: ReturnType<typeof admin>, bookingId: string, kind: string) {
  ...
  const subject = isReminder
    ? `Reminder: your YourBusiness visit is tomorrow`
    : `You're booked at YourBusiness 🌹`;
  const html =
    ...
    `<p>Your Business · Woodland Hills, CA</p>` +
  ...
  return { to, subject, html, sms: `YourBusiness: ${svc} — ${when}. Woodland Hills, CA.` };
}

...
(function () {
  ...
  /* ── Dotted surface — waving particle floors (DottedSurface port) ── */
  /* One floor per dark section instead of a fixed viewport layer:
     the artifact preview stretches the iframe to full page height,
     which breaks viewport-fixed backdrops — per-section canvases
     render identically everywhere, including phones. */
  (function () {
    var reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    var floors = [];
    [].forEach.call(document.querySelectorAll("[data-dot-floor]"), function (host) {
      var canvas = document.createElement("canvas");
      canvas.className = "dots-floor";
      host.prepend(canvas);
    ...
    function resize() {
      floors.forEach(function (f) {
        ...
        f.canvas.width = Math.max(1, f.W * dpr);
        f.canvas.height = Math.max(1, f.H * dpr);
    ...
    function drawFloor(f) {
      ...
      var AX = small ? 30 : 42;
      var AY = 30;
      var horizon = f.H * 0.42;
      /* fit the wave band inside the section, whatever its height */
      var k = Math.min(1, (f.H - horizon) / 380);
    ...
    if (reduce) {
      t = 1.7;
      floors.forEach(drawFloor); // one still frame each
    } else {
      (function frame() {
        floors.forEach(function (f) { if (f.visible) drawFloor(f); });
        t += 0.05;
